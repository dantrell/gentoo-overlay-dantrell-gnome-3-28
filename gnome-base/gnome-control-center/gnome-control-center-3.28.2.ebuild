...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-3.28.1-optional.patch
	...
	if ! use vanilla-datetime; then
		...
		eapply "${FILESDIR}"/${PN}-3.28.1-disable-automatic-datetime-and-timezone-options.patch
	fi

	if ! use vanilla-hostname; then
		# From Funtoo:
		# 	https://bugs.funtoo.org/browse/FL-1391
		eapply "${FILESDIR}"/${PN}-3.28.1-disable-changing-hostname.patch
	fi
